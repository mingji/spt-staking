...
/**
 * This is a replica of the existing deployed SPT token contract.
...
library Roles {
    ...
    function remove(Role storage role, address account) internal {
        require(account != address(0), "Address 0");
        require(has(role, account), "Doesn't have role");
...
abstract contract AdminRole is Ownable{
    ...
    modifier onlyAdmin() {
        require(isAdmin(msg.sender)|| isOwner(msg.sender), "Not admin or owner");
        _;
    }

    function isAdmin(address account) public view returns (bool) {
        return adminList.has(account);
    }
    ...
    function _removeAdmin(address account) internal {
        adminList.remove(account);
        emit AdminRemoved(account);
    }
}


abstract contract Pausable is AdminRole {
...
contract ERC20Pausable is ERC20, Pausable {
    ...
    function increaseAllowance(address spender, uint addedValue) public virtual override whenNotPaused returns (bool success) {
        return super.increaseAllowance(spender, addedValue);
    }

    function decreaseAllowance(address spender, uint subtractedValue) public virtual override whenNotPaused returns (bool success) {
...
contract SPT is ERC20Detailed, ERC20Pausable, ERC20Burnable {
    ...
    modifier notFrozen(address _holder) {
        require(!frozenAccount[_holder], "Frozen");
        _;
    }
    
    constructor() ERC20Detailed("SPT", "SPT", 18) {
        
        _mint(msg.sender, 10000000000 * (10 ** 18));
    }
    
    function balanceOf(address owner) public override(ERC20, IERC20) view returns (uint256) {
        
        uint256 totalBalance = super.balanceOf(owner);
        if( timelockList[owner].length >0 ){
            for(uint i=0; i<timelockList[owner].length;i++){
                totalBalance += timelockList[owner][i]._amount;
            }
        }
    ...
    function transfer(address to, uint256 value) public override(ERC20Pausable, ERC20, IERC20) notFrozen(msg.sender) returns (bool) {
    ...
    function transferFrom(address from, address to, uint256 value) public override(ERC20Pausable, ERC20, IERC20) notFrozen(from) returns (bool) {
    ...
    function approve(address spender, uint256 value) public override(ERC20Pausable, ERC20, IERC20) returns (bool) {
        return super.approve(spender, value);
    }

    function increaseAllowance(address spender, uint addedValue) public override(ERC20Pausable, ERC20) returns (bool success) {
        return super.increaseAllowance(spender, addedValue);
    }

    function decreaseAllowance(address spender, uint subtractedValue) public override(ERC20Pausable, ERC20) returns (bool success) {
        return super.decreaseAllowance(spender, subtractedValue);
    }
    
    function freezeAccount(address holder) public onlyAdmin returns (bool) {
        require(!frozenAccount[holder], "Frozen");
    ...
    function lock(address holder, uint256 value, uint256 releaseTime) public onlyAdmin returns (bool) {
        require(_balances[holder] >= value,"Not enough balance of holder.");
    ...
    function unlock(address holder, uint256 idx) public onlyAdmin returns (bool) {
        require( timelockList[holder].length > idx, "No lock info.");
        _unlock(holder,idx);
        return true;
    }
    ...
    function upgradeTo(address _newImplementation) public onlyOwner {
        require(implementation != _newImplementation, "Same implementation");
    ...
    function _unlock(address holder, uint256 idx) internal returns(bool) {
        ...
        _balances[holder] += releaseAmount;
    ...
    fallback() external payable {
        ...
        require(impl != address(0), "Address 0");
        assembly {
            /*
